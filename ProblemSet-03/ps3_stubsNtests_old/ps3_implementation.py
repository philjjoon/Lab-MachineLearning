...
def cv(X, y, method, params, loss_function=zero_one_loss, nfolds=10, nrepetitions=5):
    ...
    #combs = list(it.product(kernel, kernelparam, regularization))
    ...
    if len(all_loss) > 1:
        ...
        method.cvloss = all_loss[idx_min_loss]

    method.fit(X, y, params[idx_min_loss][0], params[idx_min_loss][1], params[idx_min_loss][2])
    method.predict(X)
...
class krr():
    ...
    def efficient_LOOCV(self, K, y):
        ...
        L = np.diag(L)
        n = L.shape[0]
        #C = np.logspace(-2, 2, n_grid)
        min_value = np.log10(max(0.01, mu - offset))
        ...
        if self.kernel == 'polynomial':
            ...
            #C = np.logspace(mu - offset, mu + offset, n_grid)
        ...
        for c in C:
            ...
            err = np.mean(((y - y_pred.T) / (np.ones(n) - np.diagonal(S))) ** 2 )
            #err = np.mean((y - y_pred.T) ** 2 )
            #print 'err: ', err
            err_list.append(err)
